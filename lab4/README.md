...
исходные данные в **PostgreSQL** (5 CSV) и **ClickHouse** (5 CSV), Trino объединяет их,
строит модель «звезда» в ClickHouse и считает **шесть тематических витрин** из задания.

**Шесть пунктов задания и физические таблицы.** В формулировке курса требуются *шесть отчётов по темам* (продукты, клиенты, время, магазины, поставщики, качество). Часть пунктов внутри темы требует **дополнительного среза** (например, «распределение по странам» рядом с «топ‑10 клиентов»), поэтому в схеме `reports` несколько физических таблиц на одну тему: основная витрина + вспомогательная. Дополнительно добавлены таблицы **годового тренда**, **сравнения с предыдущим месяцем** и **корреляции рейтинга с продажами** — это закрывает подпункты задания по времени и качеству, которые одной месячной таблицей не выразить.

Итого в `clickhouse.reports` **13 физических таблиц**; при проверке можно ссылаться на таблицу ниже: «тема N = строки в таблице X (и при необходимости Y)».

**Как устроен пайплайн (словами):**
...
**Про раскладку CSV (5+5).** В README курса в одном абзаце указано «первые 5 файлов в ClickHouse», в шагах 7–8 — только «по 5000 строк в каждой БД». Здесь зафиксировано: **PostgreSQL — `MOCK_DATA.csv` … `(4)`, ClickHouse — `(5)` … `(9)`** (как в шагах по количеству строк).
...
Все команды — из папки `lab4`. Используется `docker-compose` (с дефисом).
...
`dim_store` / `dim_supplier` ≈ 10000 — в CSV у каждой строки продажи своя комбинация полей магазина/поставщика, поэтому много уникальных ключей (как в lab2).
...
**Пример вывода (ожидаемые порядки величин):** `report_time_year` — **1** строка (в данных один календарный год), `report_time_period_compare` — **12** (по числу месяцев), `report_quality_correlation` — **1** строка.
